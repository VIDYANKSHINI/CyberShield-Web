...
interface RevealGalleryProps {
  items: RevealItem[];
}

// --- Hook: Robust Body Scroll Lock ---
const useScrollLock = (isOpen: boolean) => {
  useLayoutEffect(() => {
    ...
    if (isOpen) {
      // Prevent scrolling on html/body and handle layout shift
      document.body.style.overflow = "hidden";
      document.body.style.paddingRight = `${scrollbarWidth}px`;

      // If using Lenis, we often need to stop it or rely on data-lenis-prevent
      // This CSS lock is usually sufficient if the modal overlay captures pointer events
    }
...
const RevealModal = ({ item, originRect, onClose }: ModalProps) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 1. Initial State: Match the grid card exactly
      gsap.set(modal, {
        position: "fixed",
        ...
        zIndex: 60,
        backgroundColor: "#ffffff", // Ensure it starts white
      });

      // 2. Target State: Editorial Layout
      ...
      tl.to(modal, {
        ...
        height: targetHeight,
        duration: 0.8,
        ease: "power4.inOut", // "Awwwards" style ease (slow start/end, fast middle)
      });

      // Animate Content In
      // We reveal the text with a stagger effect
      if (textRef.current) {
        tl.fromTo(
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      >
        ...
        {/* Modal Layout: Split View (Image Left/Top, Content Right/Bottom) */}
        <div className="w-full h-full flex flex-col md:flex-row">
          {/* Image Section */}
          <div className="w-full md:w-1/2 h-64 md:h-full bg-zinc-50 relative overflow-hidden flex-shrink-0">
            <img
              ref={imgRef}
              src={item.imageSrc}
              alt={item.title}
          ...
          {/* Content Section */}
          {/* data-lenis-prevent is crucial for inner scrolling */}
          <div
            ref={contentRef}
          ...
          >
            ...
            >
              ...
              <div className="prose prose-zinc prose-lg text-zinc-600 leading-relaxed w-full max-w-none">
                ...
                {/* Example rich text structure for testing alignment */}
              </div>
            </div>
          </div>
        </div>
...
const RevealGallery: React.FC<RevealGalleryProps> = ({ items }) => {
  ...
  return (
    <section className="w-full bg-white text-zinc-900 py-24 px-6 md:px-12 relative">
      <div className="max-w-[1400px] mx-auto">
        {/* Section Header */}
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-zinc-200 pb-8">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-8">
          {items.map((item, index) => (
            ...
            >
              {/* Image Container */}
              {/* Aspect ratio container with overflow hidden */}
